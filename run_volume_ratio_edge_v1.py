"""
run_volume_ratio_edge_v1.py — RP-0010-FEATURE-VOLUME

První Feature Validation experiment: Volume Ratio vs 20D forward return (spojitý).
Wiring 1:1 podle run_mle_irc_robustness_grid.py.

Spuštění:
    conda activate market_research_lab
    cd C:\\Users\\stava\\Projects\\MarketResearchLab
    python run_volume_ratio_edge_v1.py
"""
...
from experiments.feature_validation.volume_ratio_edge_v1 import VolumeRatioEdge_v1
...
config = ExperimentConfig(
    ...
    parameters = {},
    notes      = "RP-0010: Volume Ratio Edge (F001, spojitý target, žádný práh)",
)
...
logger.info("=== Volume Ratio Edge (RP-0010, F001) ===")
result = runner.run(experiment=VolumeRatioEdge_v1(), config=config)
...
print(f"Spearman rho       : {m.get('spearman_rho')}  CI[{m.get('rho_ci_lo')},{m.get('rho_ci_hi')}]  sig={m.get('rho_significant')}")
print(f"H1 supported       : {m.get('H1_supported')}   (H0 rejected: {m.get('H0_rejected')})")
print(f"Kvantil spread     : {m.get('quantile_spread_top_minus_bottom_pp')}pp  (JEN interpretace)")
