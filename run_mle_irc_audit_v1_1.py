"""
run_mle_irc_audit_v1_1.py — MLE x IRC Dependency Audit v1.1.0
...
Spusteni:
    ...
    python run_mle_irc_audit_v1_1.py
"""
...
from experiments.edge_validation.mle_irc_dependency_audit_v1_1 import MLEIRCDependencyAudit_v1_1
...
# --- SPY prices --- (neni v universe, nacteme separatne)
...
spy_df = spy_df[
    (spy_df.index >= pd.Timestamp(DATE_FROM)) &
    (spy_df.index <= pd.Timestamp(DATE_TO))
]
logger.info(f"SPY nacteno: {len(spy_df)} radku")

# --- ProviderBundle ---
price_provider = MDSMPriceProvider(prices_dir=MDSM_PRICES)
...
config = ExperimentConfig(
    ...
    parameters = {},
    notes      = "MLE x IRC Dependency Audit v1.1.0 - reprodukce Test 7",
)

# Monkey-patch: po sestavení contextu vlož SPY prices
...
logger.info("=== MLE x IRC Dependency Audit v1.1.0 ===")
result = runner.run(experiment=MLEIRCDependencyAudit_v1_1(), config=config)
